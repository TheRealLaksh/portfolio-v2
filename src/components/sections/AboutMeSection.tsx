"use client";

import * as THREE from "three";
import Image from "next/legacy/image";
import { motion, circOut } from "framer-motion";
import { GlassmorphicCard } from "@/components/custom-ui/GlassmorphicCard";
import Link from "next/link";
import { SectionHeading } from "@/components/custom-ui/SectionHeading";
import { Images } from "@/lib/images";
import AnimatedBlobBackground from "@/components/custom-ui/AnimatedBlobBackground";
import { Canvas } from "@react-three/fiber";
import { Stars } from "@react-three/drei";
import {
  EffectComposer,
  Bloom,
  Vignette,
  ChromaticAberration,
  Noise,
} from "@react-three/postprocessing";
import { BlendFunction } from "postprocessing";

const AboutMeSection = () => {
  const profileImageUrl = Images.ProfilePlaceholder; // This image will need to be replaced in /lib/images.ts

  const imageContainerVariants = {
    hidden: { opacity: 0, x: -50 },
    visible: {
      opacity: 1,
      x: 0,
      transition: { duration: 0.7, ease: circOut, delay: 0.3 },
    },
  };

  return (
    <section
      id="about"
      className="py-24 sm:py-32 relative bg-gray-950 text-white overflow-hidden"
    >
      <AnimatedBlobBackground />

      <div className="container relative z-10 mx-auto px-4">
        <SectionHeading title="About Me" subtitle="My Journey" />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-start md:items-center">
          <motion.div
            className="relative"
            variants={imageContainerVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.2 }}
          >
            <div className="absolute -inset-3 sm:-inset-4 rounded-xl bg-gradient-to-br from-purple-500/30 via-pink-500/20 to-teal-500/30 blur-xl opacity-60 sm:opacity-70 transition-all duration-500 group-hover:opacity-90"></div>
            <div className="relative aspect-[3/4] sm:aspect-square rounded-xl overflow-hidden border border-zinc-800 shadow-2xl group">
              <Image
                src={profileImageUrl}
                alt="Laksh Pradhwani - Web Developer and AI/ML Engineer"
                layout="fill"
                objectFit="cover"
                className="transform transition-transform duration-500 group-hover:scale-105"
                priority
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent"></div>
              <div className="absolute bottom-0 left-0 w-full p-4 sm:p-6">
                <div className="flex items-center gap-2 bg-black/30 backdrop-blur-sm px-3 py-1.5 rounded-md w-fit">
                  <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-green-500 animate-pulse"></div>
                  <span className="text-xs sm:text-sm font-medium text-gray-100">
                    Available for work
                  </span>
                </div>
              </div>
            </div>
          </motion.div>

          <div className="space-y-6">
            <GlassmorphicCard>
              <p className="text-lg text-zinc-300 leading-relaxed lg:text-justify">
                As an aspiring AI/ML Engineer and high school student, my
                journey from a simple &quot;Hello, World!&quot; to building
                full-stack applications is driven by a passion for transforming
                creative ideas into tangible solutions. With a strong foundation
                in the <strong className="text-teal-400">MERN stack</strong>,
                I&apos;ve honed my skills through{" "}
                <strong className="font-semibold text-white">
                  three internships
                </strong>
                , applying my technical knowledge to real-world projects.
              </p>
              <p className="text-lg text-zinc-300 mt-4 leading-relaxed lg:text-justify">
                I thrive on solving complex problems, a skill sharpened through
                consistent success in tech competitions, including winning
                multiple{" "}
                <strong className="font-semibold text-white">hackathons</strong>{" "}
                and being a top performer in the{" "}
                <strong className="font-semibold text-white">
                  VVM National Science Competition
                </strong>
                . My experience as House Captain and in competitive sports has
                instilled in me a disciplined, goal-oriented approach.
              </p>
              <p className="text-lg text-zinc-300 mt-4 leading-relaxed lg:text-justify">
                When I&apos;m not coding, I enjoy contributing to open-source
                projects and attending insightful and informative workshops. I
                am actively seeking new internship opportunities to contribute
                to innovative projects and continue my growth.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mt-8 pt-6 border-t border-zinc-700/50">
                <div>
                  <div className="text-xs sm:text-sm text-white/90">Name</div>
                  <div className="font-medium text-zinc-200">
                    Laksh Pradhwani
                  </div>
                </div>
                <div>
                  <div className="text-xs sm:text-sm text-white/90">Email</div>
                  <Link
                    href="mailto:laksh.pradhwani@Gmail.com"
                    className="font-medium text-zinc-200 hover:text-teal-400 transition-colors"
                  >
                    laksh.pradhwani@Gmail.com
                  </Link>
                </div>
                <div>
                  <div className="text-xs sm:text-sm text-white/90">
                    Location
                  </div>
                  <div className="font-medium text-zinc-200">
                    Varanasi, Uttar Pradesh, India
                  </div>
                </div>
                <div>
                  <div className="text-xs sm:text-sm text-white/90">
                    Availability
                  </div>
                  <div className="font-medium text-green-400">
                    Open to Freelancing
                  </div>
                </div>
              </div>
            </GlassmorphicCard>
          </div>
        </div>
      </div>

      <div className="absolute inset-0 z-0">
        <Canvas>
          <Stars radius={50} count={2500} factor={4} fade speed={2} />
          <EffectComposer>
            <Bloom luminanceThreshold={0.2} intensity={0.8} mipmapBlur={true} />
            <ChromaticAberration offset={new THREE.Vector2(0.001, 0.001)} />
            <Noise
              premultiply
              blendFunction={BlendFunction.ADD}
              opacity={0.05}
            />
            <Vignette eskil={false} offset={0.1} darkness={0.9} />
          </EffectComposer>
        </Canvas>
      </div>
    </section>
  );
};

export default AboutMeSection;
