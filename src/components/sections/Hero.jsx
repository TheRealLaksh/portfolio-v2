...
const Hero = () => {
  ...
  // Framer Motion Hooks for Scroll 3D Effect
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start start", "end start"] // Tracking from top of viewport to when it leaves
  });

  // 1. Tilt backward (Rotate X)
  const rotateX = useTransform(scrollYProgress, [0, 1], [0, 45]); 
  // 2. Fade out opacity
  const opacity = useTransform(scrollYProgress, [0, 0.5], [1, 0]);
  // 3. Scale down slightly
  const scale = useTransform(scrollYProgress, [0, 1], [1, 0.8]);
  // 4. Move down slightly (Parallax)
  ...
  return (
    // "perspective-[1000px]" is crucial for the 3D tilt effect
    <section ref={containerRef} id="home" className="min-h-screen flex flex-col justify-center relative z-10 px-4 sm:px-0 perspective-[1200px]">
      
      <motion.div 
        style={{ rotateX, opacity, scale, y }} // Apply the 3D transforms here
        className="w-full max-w-4xl mx-auto text-center origin-center"
      >

        <h2 className="text-3xl sm:text-5xl font-bold text-slate-400 mb-8" data-aos="fade-up" data-aos-delay="100">
          Hello, I'm
        </h2>
        
        <h1 className="text-6xl sm:text-8xl font-extrabold text-white tracking-tight mb-6 drop-shadow-2xl" data-aos="fade-up" data-aos-delay="200">
          Laksh Pradhwani.
        </h1>

        <h2 className="text-3xl sm:text-5xl font-bold text-slate-400 mb-8 h-[60px] sm:h-[80px]" data-aos="fade-up" data-aos-delay="300">
          <span ref={typedEl} id="typed-text" className="text-slate-300"></span>
        </h2>

        <p className="max-w-2xl mx-auto text-slate-400 text-lg leading-relaxed mb-10" data-aos="fade-up" data-aos-delay="400">
        ...
        {/* Buttons */}
        <div className="flex flex-wrap justify-center gap-6 mb-12" data-aos="fade-up" data-aos-delay="500">
        ...
        {/* Social Links */}
        <div className="flex flex-wrap justify-center items-center gap-4 mt-8" data-aos="fade-up" data-aos-delay="600">
           {/* ... Copy your existing Social Link buttons here ... */}
           {/* GitHub */}
